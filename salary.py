...
for line in cursor:
	...
	for line in cursor:
		...
		cursor.execute("select avg(salary) from doagender where title =%s and ((percent*3200000)/((percent2*3000000)+(percent*3200000)))*100 > 70", (title[0]))
		for line in cursor:
			af = list(line)
			print 'avg fem salary for %s',(title)
			print af
		...
		for line in cursor:
			am = list(line)
			print 'avg male salary for %s',(title)
			print am
		...
		for line in cursor:
			no = list(line)
			print 'no gender data for %s emp',(title)
			print no
